...
async function getUserFromToken() {
  ...
  if (!token?.access_token) {
    throw new Error("Not authenticated. Please run 'ai login' first.");
  }
...
async function selectTools() {
  ...
  console.log(
    chalk.gray(`[DEBUG] Available tools count: ${availableTools.length}`)
  );
  console.log(
    chalk.gray(
  ...
  console.log(chalk.gray(`[DEBUG] Tool options for multiselect:`, toolOptions));
...
export async function startToolChat(conversationId = null) {
  try {
    intro(
      boxen(chalk.bold.cyan("🛠️   AI - CLI  -  Tool Calling Mode"), {
        padding: 1,
        borderStyle: "double",
        borderColor: "cyan",
      })
